...
class ColaCircular {
    ...
    Pop() {
        ...
        //BUSCAMOS SI HAY LUGARES VACIOS
        var flagVacio = true
        for (var x = 0; x < this.size; x++) {
            if (this.item[x] !== undefined) {
                flagVacio = false;
        ...
        if (flagVacio) {
            //REGRESAMOS LOS VALORES A COMO ESTABAN INICIALMENTE Y SALIMOS
            this.empty = true;
        ...
        //SI EL ATRIBUTO FRONT ES 0 LO REGRESAMOS A SUS VALORES INICIALES
        if (this.front == 0) {
            this.front = this.size - 1;

        } else {//EN CASO CONTRARIO SOLO RESTAMOS 1 A FRONT
